...
/* ── Main Component ─────────────────────────────────────────────── */
export default function FaceVerification({ idImageB64, onVerified, onError }) {
    ...
    const submitMatch = async () => {
        ...
        setStatus('checking');
        try {
            const { data } = await api.post('/verification/face-match', {
                idImage:   idImageB64,
                liveImage: preview,
            });
            setResult(data);
            ...
            setStatus('failed');
            setResult({ match: false, confidence: 0, message: 'Server error during face match.' });
            onError?.();
        }
    };
    ...
    return (
        <div style={card}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: idImageB64 ? '1fr 1fr' : '1fr', gap: '1rem' }}>
                ...
                {/* ID thumbnail for comparison */}
                {idImageB64 && (
                    <div>
                        <div style={{ fontSize: '0.68rem', fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 6 }}>ID Card (Reference)</div>
                        <div style={{ borderRadius: 10, overflow: 'hidden', background: '#0F172A', aspectRatio: '4/3' }}>
                            <img src={idImageB64} alt="ID reference" style={{ width: '100%', height: '100%', objectFit: 'contain', display: 'block' }} />
                        </div>
                    </div>
                )}
            </div>
